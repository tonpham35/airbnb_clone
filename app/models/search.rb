class Search < ApplicationRecord
	def search_listings
		...
		listings = listings.where([:smoker => true]) if smoker == true
		listings = listings.where([:pets => true]) if pets == true
		listings = listings.where([:instant_book => true]) if instant_book == true
